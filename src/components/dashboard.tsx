...
export function Dashboard({ ticker, statuses, lastRefreshAt }: DashboardProps) {
  ...
  const failedExchanges = EXCHANGES.filter((exchange) =>
    ...
    <section className="space-y-5">
      {failedExchanges.length > 0 ? (
        <div className="panel border-[color:rgba(255,111,124,0.5)] text-sm text-[var(--text-secondary)]">
          Some venues are currently unavailable:{" "}
          {failedExchanges
      ...
      <section className="panel">
        <div className="mb-4 space-y-1">
          ...
          <h3 className="text-2xl font-semibold tracking-tight text-[var(--text-primary)]">
            {ticker} Multi-Venue Depth Curves
          </h3>
          <p className="text-sm text-[var(--text-secondary)]">
            Compare bid/ask cumulative depth by venue, with per-side cap presets
            from $1K to $1M.
          </p>
        </div>
