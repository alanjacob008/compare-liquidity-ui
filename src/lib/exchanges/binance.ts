...
export function parseBinanceBook(raw: unknown): NormalizedBook {
  const data = raw as BinanceBookResponse;
  if (!data || !Array.isArray(data.bids) || !Array.isArray(data.asks)) {
    throw new Error("Invalid Binance order book response");
  }
  ...
  if (bids.length === 0 || asks.length === 0) {
    throw new Error("Binance order book is empty");
  }
